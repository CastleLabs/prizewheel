...
Version: 2.2 - Fixed Character Encoding and Critical Issues
Author: Prize Wheel System Team
Date: 2024

CRITICAL FIXES APPLIED:
1. ✅ Server-side winner determination with proper client synchronization
2. ✅ Database seeding from sample_prizes.json file  
3. ✅ Enhanced spin coordination between server and client
4. ✅ Proper winner index calculation for wheel animation targeting
5. ✅ Fixed character encoding issues
6. ✅ Added robust error handling
"""

import os
### FIX APPLIED ### - Import load_dotenv to read the .env configuration file.
from dotenv import load_dotenv

### FIX APPLIED ### - Load environment variables at the very beginning.
load_dotenv()
...
try:
    ...
    print("✅ Pi 5 GPIO support loaded successfully")
except ImportError:
    PI5_GPIO_AVAILABLE = False
    print("⚠️ GPIO not available - running in development mode")

try:
    import psutil
    PI5_MONITORING_AVAILABLE = True
    print("✅ Pi 5 system monitoring enabled")
except ImportError:
...
class Config:
    ...
    SQLALCHEMY_DATABASE_URI = os.environ.get('DATABASE_URL') or 'sqlite:///prizewheel.db'
...
logger.info("🚀 Prize Wheel System v2.2 - Pi 5 Enhanced Edition Starting")
...
class Prize(db.Model):
    ...
    weight = db.Column(db.Float, default=1.0, index=True)
    color = db.Column(db.String(7), default='#FF6B6B')
    image_path = db.Column(db.String(255))
    sound_path = db.Column(db.String(255))
    is_winner = db.Column(db.Boolean, default=True, index=True)
...
class SpinHistory(db.Model):
    ...
    system_performance = db.Column(db.String(50))
    hardware_source = db.Column(db.String(20), default='pi5')
...
class Configuration(db.Model):
    ...
    key = db.Column(db.String(100), primary_key=True, index=True)
...
def validate_audio_file(file):
    ...
    # Magic number validation for common audio formats
    audio_signatures = [
...
# CRITICAL FIX: Enhanced server-side winner calculation with validation
def calculate_winner(prizes):
    """Calculate winner server-side using weighted random selection"""
    if not prizes:
        logger.warning("No prizes available for winner calculation")
        return None
    
    # Filter enabled prizes only
    enabled_prizes = [p for p in prizes if p.enabled]
    if not enabled_prizes:
        logger.warning("No enabled prizes available")
        return None
    
    total_weight = sum(p.weight for p in enabled_prizes)
    if total_weight <= 0:
        logger.warning("Total weight is zero or negative, using random selection")
        return random.choice(enabled_prizes)
    ...
    try:
        ...
        for prize in enabled_prizes:
            ...
            if random_value <= current_weight:
                logger.info(f"🎯 Winner calculated: {prize.name} (weight: {prize.weight}, total: {total_weight})")
                return prize
        ...
        return random.choice(enabled_prizes) if enabled_prizes else None

# Calculate winner index in the prizes list
def get_winner_index(winner, prizes):
    ...
    try:
        ...
        logger.error(f"Error getting winner index: {e}")
        return 0
...
def setup_gpio():
    ...
    try:
        ...
        logger.info("✅ Pi 5 GPIO initialized successfully")
    except Exception as e:
        logger.error(f"❌ Pi 5 GPIO initialization failed: {e}")
...
def button_callback(channel):
    ...
    logger.info("🎮 Pi 5: Hardware button pressed - triggering spin")
    trigger_spin('button')

def trigger_spin(source='web'):
    """Enhanced spin trigger with server-side winner determination and proper client sync"""
    global wheel_state
    ...
    with state_lock:
        ...
        prizes = Prize.query.filter_by(enabled=True).order_by(Prize.id).all()
        if not prizes:
        ...
        # Get winner index for client animation
        ...
        wheel_state['current_winner'] = winner
    ...
    # Send comprehensive winner information to client for proper animation
    socketio.emit('spin_started', {
        ...
        'spin_duration': actual_spin_duration * 1000,
        'total_prizes': len(prizes),
        'prizes_order': [p.id for p in prizes]
    })
    ...
    logger.info(f"🎰 Pi 5: Spin started - winner: {winner.name} at index {winner_index} (duration: {actual_spin_duration:.1f}s)")
...
def complete_spin(winner, source='web', actual_duration=5.0):
    ...
    try:
        with app.app_context():
            ...
            performance_summary = f"CPU:{pi5_info.get('cpu_percent', 0):.1f}% Temp:{pi5_info.get('cpu_temperature', 0):.1f}°C"
            ...
            db.session.add(spin)
            db.session.commit()
            logger.info(f"✅ Pi 5: Spin recorded in database")
    except Exception as e:
        logger.error(f"❌ Pi 5: Failed to record spin history: {e}")
        db.session.rollback()
    ...
    socketio.emit('spin_complete', {
        ...
        'cooldown_duration': cooldown_duration * 1000
    })
    
    logger.info(f"🏆 Pi 5: Spin completed - winner: {winner.name}")
...
def set_config(key, value, category='general', description=''):
    ...
    try:
        ...
        db.session.commit()
        logger.info(f"✅ Pi 5: Config updated: {key}")
    except Exception as e:
        logger.error(f"❌ Pi 5: Failed to set config {key}: {e}")
        db.session.rollback()
...
def display():
    ...
    try:
        prizes = Prize.query.filter_by(enabled=True).order_by(Prize.id).all()
        logger.info(f"🖥️ Pi 5: Display loaded with {len(prizes)} prizes")
        return render_template('display.html', prizes=[p.to_dict() for p in prizes])
    except Exception as e:
        logger.error(f"❌ Pi 5: Display route error: {e}")
...
def health():
    ...
    try:
        ...
        return jsonify({
            ...
            'timestamp': datetime.utcnow().isoformat(),
            'database': True,
            'gpio_available': PI5_GPIO_AVAILABLE,
            'monitoring_available': PI5_MONITORING_AVAILABLE
        })
    except Exception as e:
        return jsonify({
            'status': 'unhealthy',
...
def admin_login_post():
    ...
    try:
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            session['pi5_session'] = True
            user.last_login = datetime.utcnow()
            user.last_activity = datetime.utcnow()
            user.failed_login_attempts = 0
            user.locked_until = None
            db.session.commit()
            logger.info(f"✅ Pi 5: Successful login: {username}")
            return redirect(url_for('admin_dashboard'))
        else:
            # Failed login
            if user:
                ...
                if user.failed_login_attempts >= 5:
                    ...
                    logger.warning(f"🔒 Pi 5: Account locked due to failed attempts: {username}")
                db.session.commit()
            
            logger.warning(f"❌ Pi 5: Failed login attempt: {username}")
            return render_template('login.html', error='Invalid credentials')
    except Exception as e:
        logger.error(f"❌ Pi 5: Login error: {e}")
...
def admin_logout():
    ...
    session.clear()
    logger.info(f"👋 Pi 5: User logged out: {username}")
...
def admin_dashboard():
    ...
    try:
        ...
        logger.info(f"📊 Pi 5: Dashboard accessed by {session.get('username')}")
        ...
        logger.error(f"❌ Pi 5: Dashboard error: {e}")
        return render_template('dashboard.html', 
                             total_spins=0, recent_spins=[], prizes=[], pi5_stats={})

# API Routes - Prize Management
...
def api_prizes():
    ...
    try:
        ...
        logger.error(f"❌ Pi 5: Get prizes error: {e}")
...
def upload_sound():
    ...
    if file and allowed_file(file.filename, app.config['ALLOWED_AUDIO_EXTENSIONS']):
        # Validate audio file
...
def export_csv():
    ...
    try:
        spins = SpinHistory.query.options(joinedload(SpinHistory.prize)).order_by(SpinHistory.timestamp.asc()).all()
...
def api_trigger_spin():
    ...
    try:
        ...
        logger.info(f"🌐 Pi 5: Web spin triggered from {client_ip}")
        return jsonify({
            'status': 'spinning',
            'pi5_enhanced': True,
            'timestamp': datetime.utcnow().isoformat()
        ...
        logger.error(f"❌ Pi 5: API spin error: {e}")
...
def api_health_check():
    ...
    try:
        ...
        logger.error(f"❌ Pi 5: Health check error: {e}")
        return jsonify({
            'status': 'error',
            'error': str(e),
            'pi5_enhanced': True
...
# Database seeding from sample_prizes.json
def load_sample_prizes():
    ...
    if os.path.exists(sample_file):
        try:
            ...
            logger.info(f"✅ Pi 5: Loading {len(sample_prizes)} sample prizes from {sample_file}")
            return sample_prizes
        except Exception as e:
            logger.error(f"❌ Pi 5: Failed to load {sample_file}: {e}")
    
    # Fallback to hardcoded prizes if file doesn't exist
    logger.info("📋 Pi 5: Using fallback hardcoded sample prizes")
    return [
    ...
    with app.app_context():
        try:
            ...
            if not User.query.filter_by(username='admin').first():
                ...
                logger.info("✅ Pi 5: Default admin user created")
                
            # Load sample prizes from JSON file
            if Prize.query.count() == 0:
                ...
                logger.info(f"✅ Pi 5: {len(sample_prizes_data)} sample prizes loaded from configuration")
            
            # Set default configuration
            default_configs = [
                ('spin_duration', '5', 'wheel', 'Base spin duration in seconds'),
                ('cooldown', '2', 'wheel', 'Cooldown between spins in seconds'),
                ('animation_fps', '60', 'display', 'Animation frames per second'),
            ...
            db.session.commit()
            logger.info("✅ Pi 5: Database initialized successfully with critical fixes applied")
            
        except Exception as e:
            logger.error(f"❌ Pi 5: Database initialization failed: {e}")
            db.session.rollback()
            raise
...
def signal_handler(sig, frame):
    ...
    logger.info(f"🛑 Pi 5: Received signal {sig}, shutting down...")
    try:
        if PI5_GPIO_AVAILABLE:
            GPIO.cleanup()
            logger.info("✅ Pi 5: GPIO cleanup completed")
    except:
        pass
    sys.exit(0)
...
if __name__ == '__main__':
    try:
        logger.info("🚀 Pi 5: Starting Enhanced Prize Wheel System v2.2 with CRITICAL FIXES...")
        ...
        # Setup GPIO (with graceful fallback)
        try:
            setup_gpio()
        except Exception as e:
            logger.warning(f"⚠️ Pi 5: GPIO setup failed, continuing without hardware support: {e}")
        
        # Log system information at startup
        pi5_info = get_pi5_system_info()
        if pi5_info:
            logger.info(f"🖥️ Pi 5 System Info:")
            ...
            logger.info(f"   Temperature: {pi5_info.get('cpu_temperature', 0):.1f}°C")
            logger.info(f"   Disk: {pi5_info.get('disk_percent', 0):.1f}% used")
        
        logger.info("✅ Pi 5: CRITICAL FIXES APPLIED:")
        logger.info("   🎯 Server-side winner determination with client synchronization")
        logger.info("   📋 Database seeding from sample_prizes.json file")
        logger.info("   🎮 Enhanced spin coordination between server and client")
        logger.info("   📊 Proper winner index calculation for wheel animation")
        logger.info("   🔤 Fixed character encoding issues")
        
        logger.info("✅ Pi 5: Features enabled:")
        logger.info(f"   🎮 GPIO Hardware: {PI5_GPIO_AVAILABLE}")
        logger.info(f"   📊 System Monitoring: {PI5_MONITORING_AVAILABLE}")
        logger.info(f"   🔊 Enhanced Audio Support: True")
        logger.info(f"   🖥️ Kiosk Mode Optimized: True")
        
        # Start Flask app with SocketIO
        logger.info("🌟 Pi 5: Starting Prize Wheel System with CRITICAL FIXES applied...")
        ...
        logger.error(f"❌ Pi 5: Failed to start application: {e}")
        try:
            if PI5_GPIO_AVAILABLE:
                GPIO.cleanup()
        except:
        ...
        logger.info("👋 Pi 5: Prize Wheel System shutdown complete")

# Version Information
__version__ = "2.2.0-FIXED"
__critical_fixes__ = [
    "✅ Server-side winner determination with proper client sync",
    "✅ Database seeding from sample_prizes.json configuration file", 
    "✅ Enhanced spin coordination between server and client",
    "✅ Proper winner index calculation for wheel animation targeting",
    "✅ Consistent prize ordering for reliable client-server communication",
